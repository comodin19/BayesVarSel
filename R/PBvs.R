#' Bayesian Variable Selection for linear regression models using parallel
#' computing.
#'
...
#' @param n.keep How many of the most probable models are to be kept?
#' @param n.nodes Number of nodes to be used in the computation
...
#' details.)
#' @param time.test If TRUE a preliminary test to estimate computational time
#' is performed.
#' @return \code{PBvs} returns an object of class \code{Bvs} with the following
...
#' @seealso \code{\link[BayesVarSel]{plotBvs}} for different descriptive plots
...
PBvs <-
  function(formula,
    ...
    if (n.keep > 2 ^ (p - 1) / n.nodes)
      stop(
        "The number of models to keep (n.keep) should be smaller than the total number of models divided by the number of nodes (n.nodes)"
      )
    ...
    distrib[[n.nodes]] <-
      c(n.nodes, (n.nodes - 1) * iterperproc + 1, 2 ^ (p) - 1)#How to distribute the processes among the  nodes
    ...
    if (time.test && p >= 20) {
      ...
      result <- switch(
        ...
        "gc" = .C(
          ...
          as.integer(p),
          as.integer(1),
          as.integer(2 ^ (p - 2) - 1999),
          as.integer(2 ^ (p - 2) + 2000),
          as.character(wd),
          as.double(estim.time),
          as.integer(knull)
        ),
    ...
    inclusion <-
      incl#inclusion probabilities except for the intercept
