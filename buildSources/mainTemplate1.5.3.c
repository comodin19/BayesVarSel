void tConst (char *pI[], int *pn, int *pp, int *pSAVE, int *pinicio, int *pfinal, char *homePath[], double *time)
{
	...
	//is a (p+1)-vector: position 1 contains Pr(M|dim=1)(=Pr(MNull)), position 2 contains Pr(M|dim=2),
	//...position p contains Pr(M|dim=p)(=Pr(Mfull)).
	...
	int dimensionNull=1;
	NormConstant=1.0*tpriorprob(p,dimensionNull);
	NormConstantPrior+=tpriorprob(p,dimensionNull);

	gsl_vector_set(dimension_prob, 0, 1.0*tpriorprob(p,dimensionNull));
	gsl_vector_set(incl_prob, 0, 1.0*tpriorprob(p,dimensionNull));
	
	gsl_vector_set(Who_Max_SAVE, 0, 0);
	gsl_vector_set(Max_SAVE_BF, 0, 1.0*tpriorprob(p,dimensionNull));
	...
	for (model=StartAt; model<(SAVE+StartAt-1); model++)
		...
		BF21= tBF21fun(n,k2,k0,Q);
		//printf("The Bayes factor is: %.20f\n", BF21);

		//Now the main calculations:
		unnormPostProb=tmainalgebraics(BF21, p, index, 
